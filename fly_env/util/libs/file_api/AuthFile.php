...
class AuthFile {
    ...
    private function is_image() 
    {
        ...
        if(!in_array($extension, $this->file_types())) return ['state' => false, 'payload' => 'Upload file must be an image file'];
        return ['state' => true];
    }
    ...
    public function is_present()
    {
        if(file_exists($this->image_path.$this->file::get_name())) {
            return ['state' => false, 'payload' => 'The file you are uploading already exists'];
        }
        return ['state' => true];
    }
}
